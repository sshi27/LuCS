...
template<class K, class V, uint8_t L, uint8_t CL>
...
class ControlPlaneOthello {
  template<class K1, class V1, uint8_t L1, uint8_t CL1> friend
  class DataPlaneOthello;
...
public:
  ...
  static_assert(VCL <= 60, "Value is too long. You should consider another solution to avoid memory waste. ");
...
//private:
  ...
  inline void memResize() {
    ...
    mem.resize((((uint64_t) ma + mb) * VCL + 63) / 64);
  }
...
//private:
  ...
  void resetBuildState() {
    for (uint32_t i = 0; i < ma + mb; ++i) {
      if (maintainingDP) memSet(i, randomized ? (randVal(i) & VCMASK) : 0);
    }
  ...
  template<bool fillValue, bool fillIndex>
...
//    assert(root < ma);
    ...
    do {
      ...
      for (OthelloCPCell cell = head[nid]; cell.keyId != uint32_t(-1); cell = nextKeyOfThisKey[cell.keyId]) {
        ...
        fillSingle<fillValue, fillIndex>(cell.keyId, nextNode, nid);
  ...
  int64_t fixHalfTreeDFS(uint32_t keyId, uint32_t startNode, uint32_t skippedNode) {
    ...
    uint64_t x = fillValue ? memValueGet(startNode) : 0;
    uint32_t ix = fillIndex ? indMem[startNode] : 0;
    
    fillSingle<fillValue, fillIndex>(keyId, startNode, skippedNode);
    
    x = fillValue ? (x ^ memValueGet(startNode)) : 0;  // the xor'ed value field, including digests. E must be 1, because both ends are 1
    ...
    do {
      ...
      for (OthelloCPCell cell = head[nid]; cell.keyId != uint32_t(-1); cell = nextKeyOfThisKey[cell.keyId]) {
        ...
        fixSingle<fillValue, fillIndex>(nextNode, x, ix);
  ...
  void fillValue() {
    for (uint32_t i = 0; i < ma + mb; i++) {
      if (connectivityForest.isRoot(i)) {  // we can only fix one end's value in a cc of keys, then fix the roots'
        if ((CL || randomized) && maintainingDP) {
          memSet(i, randomized ? randVal() | 1 : 1);
        }
        
        fillTreeDFS<true, true>(i);
      }
    }
  }
    ...
    if (built) {
      if (tryCount > 20) {
        cout << "Succ " << human(keyCnt) << " Keys, ma/mb = " << human(ma) << "/" << human(mb)    //
             << " keyT" << sizeof(K) * 8 << "b  valueT" << sizeof(V) * 8 << "b"     //
             << " Lvd=" << (int) VCL << " After " << tryCount << "tries" << endl;
      }
    } else {
  ...
  inline bool lookUp(const K &k, V &out) const {
    uint32_t ha, hb;
    getIndices(k, ha, hb);
    uint32_t aa = memValueGet(ha);
    uint32_t bb = memValueGet(hb);
    out = aa ^ bb;
    return true;
  }
...
public:
  ...
  inline int64_t updateValueAt(uint32_t keyId, V val) {
    ...
    if (maintainDP) {
      ...
      uint32_t ha, hb;
      getIndices(k, ha, hb);
      return fixHalfTreeDFS<true, false>(keyId, ha, hb);
    }
